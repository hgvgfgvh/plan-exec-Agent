...
type SeeAndDenseCaptionSkill struct{}

func (s *SeeAndDenseCaptionSkill) Name() string { return "SeeAndDenseCaption" }

func (s *SeeAndDenseCaptionSkill) Description() string {
	return "深度分析：对当前屏幕状态进行全方位的视觉细节描述，用于理解复杂环境"
}

func (s *SeeAndDenseCaptionSkill) Execute(ctx context.Context, args ...interface{}) ([]interface{}, error) {
	// 1. 处理可选参数 A_focus
	var focus string
	...
	if focus != "" {
		fmt.Printf("👁️  正在执行深度视觉分析，重点关注: [%s]...\n", focus)
	} else {
		fmt.Printf("👁️  正在执行全局视觉深度分析...\n")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("视觉深度分析失败: %w", err)
	}
...
func init() {
	// 将技能注册到全局管理器
	skill.GlobalManager.Regist(&SeeAndDenseCaptionSkill{})
}
